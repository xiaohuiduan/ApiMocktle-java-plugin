...
import com.apimocktle.dashboard.env.EnvironmentData
...
class EnvironmentSettingsPanel(private val project: Project) : SettingsPanel {
    ...
    private val envTableModel = ListTableModel<EnvironmentRow>(
        arrayOf(
            object : ColumnInfo<EnvironmentRow, String>("名称") {
            ...
            object : ColumnInfo<EnvironmentRow, String>("变量") {
    ...
    init {
        ...
        scopeColumn.cellEditor = DefaultCellEditor(JComboBox(arrayOf("项目", "全局")))
    ...
    private fun showAddEnvironmentDialog() {
        val dialog = EnvironmentDialog("添加环境")
    ...
    private fun showEditEnvironmentDialog(row: EnvironmentRow) {
        val dialog = EnvironmentDialog("编辑环境", row.name, row.scope, row.variables)
        if (dialog.showAndGet()) {
    ...
    private class EnvironmentDialog(
        ...
        private val scopeCombo = JComboBox(arrayOf("项目", "全局")).apply {
            selectedItem = when (initialScope) {
                EnvironmentScope.PROJECT -> "项目"
                EnvironmentScope.GLOBAL -> "全局"
            }
        }

        private val varTableModel = DefaultTableModel(arrayOf("键", "值"), 0)
        ...
        override fun createCenterPanel(): JComponent {
            ...
            val varPanel = JPanel(BorderLayout()).apply {
                border = BorderFactory.createTitledBorder("变量")
            ...
            return JPanel(BorderLayout(4, 4)).apply {
                val topPanel = JPanel(GridLayout(0, 1, 4, 4)).apply {
                    add(JPanel(BorderLayout(4, 4)).apply {
                        add(JLabel("名称："), BorderLayout.WEST)
                        add(nameField, BorderLayout.CENTER)
                    })
                    add(JPanel(BorderLayout(4, 4)).apply {
                        add(JLabel("作用域："), BorderLayout.WEST)
