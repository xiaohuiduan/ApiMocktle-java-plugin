...
import com.apimocktle.config.resolveVariables
...
class RequestExecutor(private val project: Project) : IdeaLog {
    ...
    suspend fun executeHttp(input: HttpRequestInput): RequestResult {
        return try {
            ...
            RequestResult(body = "请求已取消", isError = true)
        } catch (e: Exception) {
            LOG.warn("请求失败：${e.message}", e)
            RequestResult(body = "错误：${e.message}", isError = true)
        }
    }
